 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".cupcakes-catalog__slider")) new core(".cupcakes-catalog__slider", {
            ...
            if (document.querySelector(".main-slider")) new core(".main-slider", {
                modules: [ Navigation, Lazy, Thumb, EffectFade ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                spaceBetween: 30,
            ...
            if (document.querySelector(".thumbs-slider")) new core(".thumbs-slider", {
                modules: [ Lazy ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
...
***************************************************************************** */
        ...
        function documentActions(e) {
            const targetElement = e.target;
            if (targetElement.classList.contains("item-catalog__cart")) {
                const productId = targetElement.closest(".item-catalog").dataset.cid;
                addToCart(targetElement, productId);
                e.preventDefault();
            }
            if (targetElement.classList.contains("cart-header__icon") || targetElement.closest(".cart-header__icon")) {
                if (document.querySelector(".cart-list").children.length > 0) document.documentElement.classList.toggle("_active-cart");
                e.preventDefault();
            } else if (!targetElement.closest(".cart-header__items") && !targetElement.classList.contains("item-catalog__cart")) document.documentElement.classList.remove("_active-cart"); else if (targetElement.closest(".cart-header__close")) document.documentElement.classList.remove("_active-cart");
        ...
        function addToCart(productButton, productId) {
            if (!productButton.classList.contains("._hold")) {
                ...
                const cart = document.querySelector(`[data-cart='${productId}']`);
                const product = document.querySelector(`[data-cid='${productId}']`);
                const productImage = product.querySelector(".item-catalog__image-ibg");
        ...
        function updateCart(productButton, productId, productAdd = true) {
            ...
            if (productAdd) {
                ...
                if (!cartProduct) {
                    const product = document.querySelector(`[data-cid="${productId}"]`);
                    const cartProductImage = product.querySelector(".item-catalog__image-ibg").innerHTML;
                    const cartProductName = product.querySelector(".item-catalog__name").innerHTML;
                    const cartProductInfo = product.querySelector(".item-catalog__info").innerHTML;
                    const cartProductCupNum = product.querySelector(".quantity__item").value;
                    const cartProductPrice = product.querySelector(".item-catalog__price").innerHTML;
                    const cartProductItemPrice = cartProductPrice.replace(/\D/g, "");
                    const cartPrice = cartProductItemPrice * cartProductCupNum;
                    const cartProductContent = `\n\t\t\t<div class="cart-list__body">\n\t\t\t\t<div class="cart-list__image-ibg">${cartProductImage}</div>\n\t\t\t\t<div class="cart-list__cupcake-about">\n\t\t\t\t\t<div class="cart-list__name">${cartProductName}</div>\n\t\t\t\t\t<div class="cart-list__info">${cartProductInfo}</div>\n\t\t\t\t</div>\t\n\t\t\t\t<div class="quantity quantity_cart">\n\t\t\t\t<div class="quantity__button quantity__button_plus"></div>\n\t\t\t\t<div class="quantity__input">\n\t\t\t\t\t<input autocomplete="off" type="text" name="form[]" value="${cartProductCupNum}">\n\t\t\t\t</div>\n\t\t\t\t<div class="quantity__button quantity__button_minus"></div>\n\t\t\t</div>\n\t\t\t\t<div hidden class="cart-list__quantity">Quantity: <span>1</span></div>\n\t\t\t\t<div class="cart-list__price">${cartPrice}&nbsp;&euro;</div>\n\t\t\t\t<a href="#" class="cart-list__delete"></a>\n\t\t\t</div>\n\t\t\t`;
        ...
        const script_element = document.getElementById("phone");
        const elementPopup = document.getElementById("popup-phone");
        var maskOptions = {
            mask: "{8}(000)000-000",
            lazy: false
        };
        new IMask(script_element, maskOptions);
        new IMask(elementPopup, maskOptions);
        window["FLS"] = true;
        isWebp();
        menuInit();
        tabs();
